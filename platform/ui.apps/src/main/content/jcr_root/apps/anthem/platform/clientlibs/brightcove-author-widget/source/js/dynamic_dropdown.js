(function($, $document) {
    ...
    $document.on("dialog-ready", function(e) {
        ...
        if (dialogRes && dialogRes.val() && dialogRes.val().indexOf('hero-overlay') >= 0 ) {
            adjustLayoutHeight();

            var accountSelector =  $("[name='" + ACCOUNTID +"']").get(0);
            var contentSelector =  (dialogRes.val() == RES_PLAYER_COMPONENT) ? $("[name='" + PLAYLISTS +"']").get(0) : $("[name='" + VIDEOS +"']").get(0)
